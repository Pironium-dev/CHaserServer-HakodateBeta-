...
class Window(tk.Frame):
    ...
    def write_screen(self):
        ...
        self.radio_box_client = tk.Radiobutton(
            self.radio_frame, variable=self.radio_box_var, value=1, 
        )
        ...
        self.check_button_symmetry = tk.Checkbutton(self.setting_frame, text="対称", variable=self.is_symmetry)

        self.turn_var = tk.StringVar(value='100')
        self.turn_label = tk.Label(self.setting_frame, text="TURN")
        self.turn_spinbox = tk.Spinbox(self.setting_frame, width=5, from_=1, to=999, textvariable=self.turn_var)

        self.loading_button = tk.Button(self.setting_frame, text="読み込み", command=self.load)
        self.write_button = tk.Button(self.setting_frame, text='書き込み', command=self.save)
    ...
    def save(self):
        with open('Config.dt', 'r') as f:
            config = json.load(f)
        file = asksaveasfile('w', initialdir=config['StagePath'], filetypes=[('マップ', '.json')], defaultextension='json')
        if file is not None:
            d = {'Map': self.convert(self.map), 'Turn': int(self.turn_var.get()), 'Cool': self.place_cool, 'Hot': self.place_hot}
            json.dump(d, file)
    ...
    def place_chip(self, x, y, kind):
        match kind:
            case 1:
                self.canvas.create_image(15 + x * 25, 15 + y * 25, image=self.cool_image, tag='client')
                x, y = self.symmetry_place(x, y)
                self.canvas.create_image(15 + x * 25, 15 + y * 25, image=self.hot_image, tag='client')
            case 2:
                self.canvas.create_image(15 + x * 25, 15 + y * 25, image=self.wall_image, tag=f'{x}_{y}')
            case 3:
                self.canvas.create_image(15 + x * 25, 15 + y * 25, image=self.item_image, tag=f'{x}_{y}')
